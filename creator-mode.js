 ...
 * 
 * Provides a visual grid overlay (A0-A5, B0-B5, etc.) for precise element identification.
 * Toggle with: Press 'G' key or click the toggle button
 * 
 * Usage:
 * - Enable: Press 'G' or click toggle button
 * - Reference elements: "A2-B3" (from A2 to B3), or "B1" (single cell)
 * - Sections are also labeled with data-coord attributes
 */
...
(function() {
  ...
  // Can be set via: window.creatorModePrefix = 'L' before loading
  ...
  function calculateGridRows() {
    ...
    // Calculate rows needed to cover full document height
  ...
  function getElementAtCoord(coord) {
    // Accept both prefixed (LA1) and non-prefixed (A1) coordinates
    let baseCoord = coord;
